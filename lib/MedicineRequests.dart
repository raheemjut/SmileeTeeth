...
import 'package:finalproject/showImage.dart';
...
class Spacecraft {
  final String id;
  final String username, name, email, phone, address, cnic, doctor, image, orderdate, orderimage;
...
class MedicieneRequest extends StatefulWidget {
  @override
  _MedicieneRequestState createState() => _MedicieneRequestState();
}

class _MedicieneRequestState extends State<MedicieneRequest> {
  ...
  List<Spacecraft> filteredorders = List();
   Future<List<Spacecraft>> downloadJSON() async {
    ...
    final response =
        await http.post(ApiUrls.fetchorders + "?nochache=1", body: {
   ...
   Future<List> updateAppointment(String appointmentid, String status) async {
    final response = await http.post(ApiUrls.updateorder, body: {
      "aptid": appointmentid,
      "status":status
    ...
    var arr = message.split('*');
     if (arr[0] == "pass") {
      if (response.statusCode == 200) {
        setState(() {
          showMessage("Order Updated", Colors.green, Colors.white);
 Navigator.pushReplacement(
        context,
        MaterialPageRoute(
            builder: (BuildContext context) => MedicieneRequest()));
              ...
              children: [
                Row(
                  ...
                  Column(
                    ...
                    mainAxisAlignment: MainAxisAlignment.start,
                    children: [
                      Text("Receiver Name: " +filteredorders[index].name,
                ...
                InkWell(
                  ...
                  MaterialPageRoute(builder: (context)=>ShowImage(
                    imagelink: filteredorders[index].orderimage,
                  ))
                    );
                  },
                  child: Container(
                ...
                InkWell(
                   onTap: () {
                     _makePhoneCall('tel:+'+filteredorders[index].phone);
